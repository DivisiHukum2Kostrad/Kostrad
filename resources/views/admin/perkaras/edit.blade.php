...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">Edit informasi perkara {{ $perkara->nomor_perkara }}</p>
        </div>

        <div class="bg-white rounded-xl shadow-lg p-8">
            <form method="POST" action="{{ route('admin.perkaras.update', $perkara) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nomor_perkara" value="{{ old('nomor_perkara', $perkara->nomor_perkara) }}"
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="jenis_perkara" value="{{ old('jenis_perkara', $perkara->jenis_perkara) }}"
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select name="priority"
                            ...
                            required>
                            <option value="Low" {{ old('priority', $perkara->priority) === 'Low' ? 'selected' : '' }}>
                                Rendah</option>
                            <option value="Medium"
                                {{ old('priority', $perkara->priority) === 'Medium' ? 'selected' : '' }}>Sedang</option>
                            <option value="High" {{ old('priority', $perkara->priority) === 'High' ? 'selected' : '' }}>
                                Tinggi</option>
                            <option value="Urgent"
                                {{ old('priority', $perkara->priority) === 'Urgent' ? 'selected' : '' }}>Mendesak</option>
                        </select>
                        @error('priority')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_perkara"
                            value="{{ old('tanggal_perkara', $perkara->tanggal_perkara?->format('Y-m-d')) }}"
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="date" name="tanggal_masuk"
                            value="{{ old('tanggal_masuk', $perkara->tanggal_masuk?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_selesai"
                            value="{{ old('tanggal_selesai', $perkara->tanggal_selesai?->format('Y-m-d')) }}"
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="date" name="deadline"
                            value="{{ old('deadline', $perkara->deadline?->format('Y-m-d')) }}"
                ...
                <div class="mb-6">
                    ...
                    <select name="status"
                        ...
                        required>
                        <option value="Proses" {{ old('status', $perkara->status) === 'Proses' ? 'selected' : '' }}>Proses
                        </option>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="keterangan" rows="4"
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-green-800 focus:outline-none @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $perkara->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="tags"
                        value="{{ old('tags', is_array($perkara->tags) ? implode(', ', $perkara->tags) : '') }}"
                ...
                <div class="mb-6">
                    ...
                    @if ($perkara->file_dokumentasi)
                        <div class="mb-2 text-sm text-gray-600">
                            File saat ini: <a href="{{ Storage::url($perkara->file_dokumentasi) }}" target="_blank"
                                class="text-green-800 hover:underline">{{ basename($perkara->file_dokumentasi) }}</a>
                        </div>
                    @endif
                    ...
                    <p class="mt-1 text-xs text-gray-500">Upload file baru untuk mengganti file yang ada</p>
